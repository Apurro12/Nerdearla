...
def reject_tool_call_node(state: State):
    ...
    return {"messages": {
                ...
                'content': 'Tool call not allowed, no permission granted by the user.'
            }}
...
async def run_agent(message: str, _history = []):
    yielded_response = ""
    async for async_stream_response in graph.astream({"messages": [message]}, config):
